...
t_eval = np.arange(start=0, stop=tf, step = dt/base_propagator.TU) if dt is not None else None
...
ax.plot(0,0,0, lw=0)
for idx in range(num_MCs):
    if dt is None: ax.plot(MC_inertial[idx][:, 0], MC_inertial[idx][:, 1], MC_inertial[idx][:, 2], lw=1, color=hsv_to_hex((360*idx/num_MCs, 0.75,1)))
    else: ax.scatter(MC_inertial[idx][:, 0], MC_inertial[idx][:, 1], MC_inertial[idx][:, 2], lw=1, color=hsv_to_hex((360*idx/num_MCs, 0.75,1)), s=1)
ax.plot(base_inertial[:, 0], base_inertial[:, 1], base_inertial[:, 2], lw=1, color='black')
...
for idx in range(num_MCs):
    if dt is None: ax2.plot(MC_CR3BP[idx][:, 0], MC_CR3BP[idx][:, 1],MC_CR3BP[idx][:, 2], lw=1, c=hsv_to_hex((360*idx/num_MCs, 0.75,1)))
    else: ax2.scatter(MC_CR3BP[idx][:, 0], MC_CR3BP[idx][:, 1],MC_CR3BP[idx][:, 2], lw=1, c=hsv_to_hex((360*idx/num_MCs, 0.75,1)), s=1)
