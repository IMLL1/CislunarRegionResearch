...
ax.plot(0,0,0, lw=0)
for idx in range(num_MCs):
    ax.scatter(MC_inertial[idx][:, 0], MC_inertial[idx][:, 1], MC_inertial[idx][:, 2], lw=1, color=hsv_to_hex((360*idx/num_MCs, 0.75,1)), s=1)
...
for idx in range(num_MCs):
    ax2.scatter(MC_CR3BP[idx][:, 0], MC_CR3BP[idx][:, 1],MC_CR3BP[idx][:, 2], lw=1, c=hsv_to_hex((360*idx/num_MCs, 0.75,1)), s=1)
